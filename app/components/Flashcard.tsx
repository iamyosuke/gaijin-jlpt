...
export default function Flashcard({ levelId }: FlashcardProps) {
  ...
  return (
    <div className="flex flex-col h-screen fixed inset-0 m-auto p-4">
      <Progress
        value={words.length > 0 ? ((currentIndex + 1) / words.length) * 100 : 0}
        className="w-full"
      />
                ...
                {!isFlipped ? (
                  <>
                    <h2 className="text-4xl sm:text-6xl mb-4">
                      {currentWord.text || currentWord.furigana}
                    </h2>
